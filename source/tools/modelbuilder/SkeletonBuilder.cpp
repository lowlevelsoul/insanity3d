...
//======================================================================================================================
SkeletonBuilder::SkeletonBuilder( const Options & options ) {
    m_options = options;
}
...
void SkeletonBuilder::Write( ToolMemStream & str, ToolSkeleton * skel ) {
    ...
    i3d::SkeletonStream header;
    memset( &header, 0, sizeof(header) );
    header.m_version = i3d::SkeletonStream::VERSION;
...
void SkeletonBuilder::BuildJointList( ToolSkeleton * skeleton, bool ignoreAttachments ) {
    ...
    if ( m_options.m_rootJointName.empty() == true) {
        AddJoint( skeleton->m_root, ignoreAttachments );
    }
    else {
        ToolJoint * rootJoint = skeleton->FindJoint( m_options.m_rootJointName.c_str() );
        XE_ERROR(rootJoint == nullptr, "Root joint '%s' was not found in skeleton",  m_options.m_rootJointName.c_str() );
    }
	...
	for ( auto j : m_joints ) {
		if ( j->m_parent == nullptr ) {
            ...
            i3d::Matrix4 mat = j->m_transform;
            mat.m_rows[3] = i3d::Vector4( i3d::Vector3( mat.m_rows[3] ) * m_options.m_scale, 1);
            ...
            i3d::Matrix4 global =  mat * m_globalMats[ parentIndex ];
...
void SkeletonBuilder::AddJoint( ToolJoint * joint, bool ignoreAttachments ) {
    ...
    if ( m_options.m_pruneMixamoMarkers == true && addJoint == true ) {
...
void SkeletonBuilder::WriteJointNames( ) {
    ...
    for ( auto j : m_joints ) {
        ...
        uint64_t hash = i3d::fh64::CalcFromString( j->m_name.c_str() );
...
//======================================================================================================================
void SkeletonBuilder::WriteHeader( ToolMemStream & str,  i3d::SkeletonStream & header ) {
