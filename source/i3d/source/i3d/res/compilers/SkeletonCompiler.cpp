...
namespace xe {
    RTTI_CLASS_BEGIN( SkeletonCompiler )
    ...
    RTTI_CLASS_END( SkeletonCompiler )
    ...
    //======================================================================================================================
    void SkeletonCompiler::Write( ToolMemStream & str, ToolSkeleton * skel ) {
        ...
        xe::SkeletonStream header;
        memset( &header, 0, sizeof(header) );
        header.m_version = xe::SkeletonStream::VERSION;
        header.m_jointCount = (uint32_t) m_joints.GetSize();
    ...
    //======================================================================================================================
    void SkeletonCompiler::BuildJointList( ToolSkeleton * skeleton, bool ignoreAttachments ) {
        // Recursively add the joints
        if ( m_rootJointName.IsEmpty() == true) {
            AddJoint( skeleton->m_root, ignoreAttachments );
        }
        else {
            ToolJoint * rootJoint = skeleton->FindJoint( m_rootJointName.GetCStr() );
            XE_ERROR(rootJoint == nullptr, "Root joint '%s' was not found in skeleton",  m_rootJointName.GetCStr() );
        }
        ...
        for ( auto j : m_joints ) {
            if ( j->m_parent == nullptr ) {
                ...
                xe::Matrix4 mat = j->m_transform;
                mat.m_rows[3] = xe::Vector4( xe::Vector3( mat.m_rows[3] ) * m_scale, 1);
                ...
                xe::Matrix4 global =  mat * m_globalMats[ parentIndex ];
    ...
    //======================================================================================================================
    void SkeletonCompiler::AddJoint( ToolJoint * joint, bool ignoreAttachments ) {
        ...
        if (  addJoint ) {
            ...
            int32_t index = ( int32_t ) m_joints.GetSize();
            uint64_t hash = xe::fh64::CalcFromString( joint->m_name.c_str() );
            ...
            if ( joint->m_parent != nullptr ) {
                ...
                parent->m_children.PushBack( copyJoint );
                copyJoint->m_parent = parent;
            }
            
            m_jointMap[ hash ] = index;
            m_joints.PushBack( copyJoint );
    ...
    //======================================================================================================================
    int32_t SkeletonCompiler::FindJoint( const char * name ) {
        uint64_t hash = xe::fh64::CalcFromString( name );
    ...
    //======================================================================================================================
    void SkeletonCompiler::WriteJointInfo() {
    ...
    //======================================================================================================================
    void SkeletonCompiler::WriteJointNames( ) {
        ...
        for ( auto j : m_joints ) {
            ...
            uint64_t hash = xe::fh64::CalcFromString( j->m_name.c_str() );
    ...
    //======================================================================================================================
    uint32_t SkeletonCompiler::WriteString( const char * name ) {
    ...
    //======================================================================================================================
    void SkeletonCompiler::WriteHeader( ToolMemStream & str,  xe::SkeletonStream & header ) {
